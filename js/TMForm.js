...
$(window).load(function(){
	...
	$('#contact-form-bottom').TMForm({
		recaptchaPublicKey:'6LeZwukSAAAAAG8HbIAE0XeNvCon_cXThgu9afkj'		
	})
})

function gtag_report_conversionTour(url) {
  ...
  gtag('event', 'conversion', {
      'send_to': 'AW-864100403/kSgbCIKa24wBELPAhJwD',
...
function gtag_report_conversionContact(url) {
  ...
  gtag('event', 'conversion', {
      'send_to': 'AW-864100403/Hu2xCMuGuX4Qs8CEnAM',
      'event_callback': callback
  });
  return false;
}
				...
				if(!$('label.'+opt.invalidClass+',label.'+opt.emptyClass,form).length&&!form.hasClass(opt.processingClass)){
					...
					$this.ajaxSubmit(function(e,d,a,c){
						...
						if(e=='success'){	
							...
							if (form.context.id == 'contact-form-bottom') {
								gtag_report_conversionContact();
								window.location = "https://www.flatsatfoxhill.com/contact-sent.html";
									
							} else {
						gtag_report_conversionTour();
						window.location = "https://www.flatsatfoxhill.com/message-sent.html";
								}
							form
